...
import { buildUploadsFromFormData, createSurveyWithServiceRole, getNextSurveyId } from "@/lib/supabase/surveys-server"
...
export async function POST(request: Request) {
  try {
    const actor = await assertModuleAction(request, "surveys", "create")
    if (!["admin", "manager", "supervisor", "surveyor", "engineer"].includes(actor.role)) {
      throw new Error("Forbidden: role cannot create surveys")
    }
    ...
    const survey = await createSurveyWithServiceRole(id, input, uploadsWithUrls, siteDetails, actor.userId)
