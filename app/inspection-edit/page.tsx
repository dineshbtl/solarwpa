...
function InspectionEditContent() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      router.push(`/inspection-details?id=${id}`)
    } catch (e) {
      toast({
  ...
  return (
    <div className="min-h-screen">
      <main className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
        <Link href={`/inspection-details?id=${id}`}>
        ...
        <Card className="border-border bg-card shadow-sm rounded-xl">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
              <Button type="button" variant="outline" onClick={() => router.push(`/inspection-details?id=${id}`)}>
                Cancel
              </Button>
...
export default function EditInspectionPage() {
  return (
    <Suspense fallback={<InspectionEditPageSkeleton />}>
